...
```
URL:           https://dev-api.immunotrack.ai/.well-known/assetlinks.json
...
```
URL:           https://dev-api.immunotrack.ai/.well-known/apple-app-site-association
...
```typescript
const deepLink = `https://dev-api.immunotrack.ai/invite?code=${raw}`;
...
> `https://dev-api.immunotrack.ai/invite?code=…` URL before the browser even
...
### Android App Links
- [ ] `https://dev-api.immunotrack.ai/.well-known/assetlinks.json` returns 200
...
- [ ] Tap `https://dev-api.immunotrack.ai/invite?code=TESTCODE` on device (app installed) → opens app
- [ ] Tap same link (app not installed) → goes to Play Store

### iOS Universal Links
- [ ] `https://dev-api.immunotrack.ai/.well-known/apple-app-site-association` returns 200
- [ ] Content-Type is `application/json`
- [ ] Team ID + bundle ID match exactly
- [ ] Test with: WWDR's "App Link Validator" or `swcutil display -f` on Mac
- [ ] Tap `https://dev-api.immunotrack.ai/invite?code=TESTCODE` in iOS Mail (app installed) → opens app
