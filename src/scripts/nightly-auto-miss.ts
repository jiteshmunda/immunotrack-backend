  ...
  try {
    ...
    for (const patient of activePatients) {
      ...
      for (const med of activeMeds) {
        ...
        const diffTime = startOfTarget.getTime() - startDate.getTime();
        const diffDays = Math.floor(diffTime / (1000 * 60 * 60 * 24));

        if (diffDays < 0) {
          continue; // Medication started after today
        }

        if (med.endDate && targetDateStr > med.endDate) {
            ...
            const logsYesterday = await db
              ...
              .where(
                and(
                  ...
                  between(schema.medicationLogs.loggedAt, startOfTarget, endOfTarget)
                )
              );
              ...
              await db.transaction(async (tx) => {
                for (let i = 0; i < deficit; i++) {
                  ...
                  await tx.insert(schema.medicationLogs).values({
                    ...
                    loggedAt: endOfTarget, // Backdate to end of target day
                  });
                  ...
                  await tx.insert(schema.missedMedicationLogs).values({
                    ...
                    missedDate: targetDateStr,
                    missedTime: endOfTarget,
        ...
        // 2. Low-Frequency / Weekly / Monthly Medications
          ...
          if (diffDays >= 0 && (diffDays + 1) % lookbackDays === 0) {
            ...
            windowStart.setDate(windowStart.getDate() - lookbackDays + 1); // +1 because today is the last day of window
            ...
            const logsInWindow = await db
              ...
              .where(
                and(
                  eq(schema.medicationLogs.medicationId, med.id),
                  between(schema.medicationLogs.loggedAt, windowStart, endOfTarget)
                )
              );
